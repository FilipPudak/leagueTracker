...
| `Sessions` | `A` token (UUID, per device) · `B` playerId · `C` deviceId · `D` email (lowercased, trimmed) · `E` created timestamp (immutable, `YYYY-MM-DD`) · `F` last active timestamp (rolling, full ISO, refreshed on each vote/link). Sessions expire after 90 days of inactivity (checked against `F`, falling back to `E` for pre-migration rows). Auto-created on first use if the sheet is missing. A row is deleted when that device is unlinked, or lazily GC'd when a stale token (e.g. after an admin unclaim or TTL expiry) is presented on a request. Players col D (email claim) is kept on unlink so the player stays claimed by their email. |
